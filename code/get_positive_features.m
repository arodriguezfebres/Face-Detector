...
for index = 1:num_images
    myImage = im2single(imread([train_path_pos, '/', image_files(index).name]));
    myHOG = vl_hog(myImage, feature_params.hog_cell_size);
    features_pos(index,:) = reshape(myHOG, 1, (feature_params.template_size / feature_params.hog_cell_size)^2 * 31);
end
